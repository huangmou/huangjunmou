...
public class CleanableAutoCompleteTextView extends AutoCompleteTextView {
	...
	private void initAutoComplete() {
		SharedPreferences sp = context.getSharedPreferences("mobilePhone", 0);
		String longhistory = sp.getString("history", "没有记录");
		String[] hisArrays = longhistory.split(",");
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(context,
				android.R.layout.simple_dropdown_item_1line, hisArrays);
		// 只保留最近的50条的记录
		if (hisArrays.length > 70) {
			String[] newArrays = new String[70];
			System.arraycopy(hisArrays, 0, newArrays, 0, 70);
...
//		setCompletionHint("最近的5条记录");
	...
	private void saveHistory(String field) {
		...
		SharedPreferences sp = context.getSharedPreferences("mobilePhone", 0);
		String longhistory = sp.getString(field, "没有记录");
	...
	private class FocusChangeListenerImpl implements OnFocusChangeListener {
		...
		public void onFocusChange(View v, boolean hasFocus) {
			CleanableAutoCompleteTextView view = (CleanableAutoCompleteTextView) v;
			isHasFocus = hasFocus;
			if (isHasFocus) {
